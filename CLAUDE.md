...
- **"More work" row (`components/Projects/ProjectRow`):** the hover expansion must never animate layout (flex-grow/width) — pointer-driven shifts count toward field CLS (`hadRecentInput` is false for hover). Every slot is absolutely positioned at the expanded width; the visible tile is `translateX` + `clip-path: inset(... round 16px)` driven by `data-pos` (idle/before/expanded/after), the poster/glyph are translated to match, and the shadow is a `drop-shadow` filter on the slot. Cloudinary posters use a per-item `loader` (up to `w_1200`, Dropper carries `c_fill,g_south,ar_1.77` to drop the recording's white window chrome). Touch/narrow (`max-width: 1023px`, `hover: none`) renders every tile open on purpose.
